...
export function insideCounter (subSegGroup_ary, beltSegGroup_ary, dataPts,segNumbX,segNumbY,slidingNumb,scaleFactor = 2,len,minmax){
        // for each subseg
        ...
        //newDensitySub = weightedMean(densityBystripe[i],areaBystripe[i],slidingNumb)
          newDensitySub = slidingCalSum(segNumbX,slidingNumb,densityBystripe[i])
        ...
        //calculcate the perpSliding
        let perpSliding = slidingNumb* segNumbY / segNumbX * len / minmax[1]
        console.log([segNumbY,segNumbX,len,minmax[1]]);
        perpSliding = Math.round(perpSliding)
        if (perpSliding %2 === 1) {
          perpSliding = perpSliding
        }
        else {
            perpSliding = perpSliding + 1
        }
        console.log(perpSliding);

        let perpenDensity=[];
        for (var i = 0; i < densityGroup.length; i++) {
          // console.log(i);
          let result = slidingCalSumPerp(perpSliding,densityGroup[i])
